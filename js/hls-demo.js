(function (factory) {
	...
	function createTestStreamWithConfig(target, config) {
		...
		var testStream = createTestStream(
			target.url,
			target.description,
			target.live,
			target.abr,
	...
	(function (exports) {
		(function (define) {
			define([], function () {
				...
				var unpack = function (packed, options) {
					...
					return (function recursiveUnpackerParser() {
						...
						options.verbose &&
							console.log('Reading collection type ' + (type === '$' ? 'object' : 'Array'));
			...
			for (var key in jsonpack) exports[key] = jsonpack[key];
		});
	})(main);
			...
			(function (global, factory) {
				...
				function endOf(units) {
					...
					switch (units) {
						...
						case 'isoWeek':
							time =
								startOfDate(this.year(), this.month(), this.date() - (this.isoWeekday() - 1) + 7) -
								1;
							break;
						case 'day':
						case 'date':
				...
				/** @lends Chart */ {
					...
					updateHoverStyle: function (elements, mode, enabled) {
						...
						for (i = 0, ilen = elements.length; i < ilen; ++i) {
							element = elements[i];
							if (element) {
								this.getDatasetMeta(element._datasetIndex).controller[prefix + 'HoverStyle'](
									element
								);
							}
						}
				...
				fit: function () {
					...
					if (tickOpts.display && display) {
						...
						if (isHorizontal) {
							...
							me.paddingRight =
								Math.max(((paddingRight - offsetRight) * me.width) / (me.width - offsetRight), 0) +
								3;
						} else {
			...
			if (!isNaN(val.min) && !isNaN(val.max)) {
				...
				if (isFragment || isCue) {
					if (drawText) {
						...
						range =
							range ||
							scale.getValueForPixel(chartArea.right) - scale.getValueForPixel(chartArea.left);
						if (range > 300 || isCue) {
							view.borderWidth = 0;
						}
					}
	...
	function hhmmss(value, fixedDigits) {
		...
		return (
			...
			pad(m, 2) +
			':' +
			pad(s.toFixed(fixedDigits), fixedDigits ? fixedDigits + 3 : 2)
		).replace(/^(?:0+:?)*(\d.*?)(?:\.0*)?$/, '$1');
	}
	...
	var TimelineChart = /*#__PURE__*/ (function () {
		...
		_proto.update = function update() {
			...
			if (
				this.hidden ||
				!((_this$chart$ctx = this.chart.ctx) != null && _this$chart$ctx.canvas.width)
			) {
				return;
			}
		...
		_proto.resize = function resize(datasets) {
			...
			if (datasets != null && datasets.length) {
				...
				var height =
					...
					5;
				var container =
					(_this$chart$canvas = this.chart.canvas) == null
						? void 0
						: _this$chart$canvas.parentElement;
				if (container) {
		...
		_proto.updateLevelOrTrack = function updateLevelOrTrack(details) {
			...
			if (!levelDataSet) {
				levelDataSet = arrayFind(datasets, function (dataset) {
					...
					return (
						((_details$fragments$ = details.fragments[0]) == null
							? void 0
							: _details$fragments$.level) === dataset.level
					);
				});
			}
			if (!levelDataSet) {
		...
		_proto.setTextTracks = function setTextTracks(textTracks) {
			...
			[].forEach.call(textTracks, function (textTrack, i) {
				...
				var data = [];
				labels.push(
					(textTrack.name || textTrack.label) + ' ' + textTrack.kind + ' (' + textTrack.mode + ')'
				);
				datasets.push(
					datasetWithDefaults({
						data: data,
	...
	function getPlaylistStart(details) {
		var _details$fragments;
		return (_details$fragments = details.fragments) != null && _details$fragments.length
	...
	var playerResize = function playerResize() {
		...
		$('#currentSize').html(
			Math.round(bounds.width * 10) / 10 + ' x ' + Math.round(bounds.height * 10) / 10
		);
	...
	function loadSelectedStream() {
		...
		hls.on(Hls.Events.FRAG_BUFFERED, function (eventName, data) {
			var stats =
				data.part && data.part.stats && data.part.stats.loaded ? data.part.stats : data.frag.stats;
			...
			var event = {
				...
				bw: Math.round(
					(8 * data.stats.total) / (data.stats.buffering.end - data.stats.loading.start)
				),
			...
			var bitrate = Math.round(
				(8 * data.stats.total) / (data.stats.buffering.end - data.stats.loading.first)
			);
		...
		if (r) {
			...
			if ($('#statsDisplayTab').is(':visible')) {
				var log =
					...
					'\nPlayed: ' +
					timeRangesToString(video.played) +
					'\n';
				if (hls.media) {
					for (var type in tracks) {
						log +=
							...
							type +
							' contains:' +
							timeRangesToString(tracks[type].buffer.buffered) +
							'\n';
					}
					...
					if (videoPlaybackQuality && typeof videoPlaybackQuality === typeof Function) {
						...
						log +=
							'Corrupted frames: ' + video.getVideoPlaybackQuality().corruptedVideoFrames + '\n';
	...
	function updateLevelInfo() {
		...
		var htmlCurrentLevel = getLevelButtonHtml(
			'currentLevel',
			levels,
			'hls.$1=$2',
		...
		var htmlCapLevel = getLevelButtonHtml(
			'autoLevelCapping',
			levels,
	...
	function onDemoConfigChanged(firstLoad) {
		...
		var permalinkURL =
			baseURL + '?src=' + encodeURIComponent(streamURL) + '&demoConfig=' + serializedDemoConfig;
	...
	function toggleTab(btn, dontHideOpenTabs) {
		...
		var modifierPressed =
			dontHideOpenTabs || (self.event && (self.event.metaKey || self.event.shiftKey));
		if (!modifierPressed) {
			hideAllTabs();
		}
		if (modifierPressed) {
