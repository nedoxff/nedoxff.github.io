(function __HLS_WORKER_BUNDLE__(__IN_WORKER__) {
	(function (global, factory) {
				...
				},
				{
					...
					get: function get() {
						return (
							...
							isFiniteNumber(this.startDate.getTime()) &&
		...
		var Fragment = /*#__PURE__*/ (function (_BaseSegment) {
			...
			_proto2.setElementaryStreamInfo = function setElementaryStreamInfo(
				type,
				startPTS,
				endPTS,
				startDTS,
						...
						if (!this._decryptdata && this.levelkeys && !this.levelkeys.NONE) {
							...
							if (_key2) {
								...
								if (keyFormats.length === 1) {
									return (this._decryptdata = this.levelkeys[keyFormats[0]].getDecryptData(
										this.sn
									));
								}
							}
		...
		var LevelDetails = /*#__PURE__*/ (function () {
			...
			_proto.reloaded = function reloaded(previous) {
				...
				this.updated =
					this.endSN !== previous.endSN || !!partIndexDiff || !!partSnDiff || !this.live;
				this.advanced =
					this.endSN > previous.endSN || partSnDiff > 0 || (partSnDiff === 0 && partIndexDiff > 0);
		...
		function parseStsd(stsd) {
			...
			switch (codec) {
				...
				case 'vp09': {
					...
					codec +=
						'.' +
						addLeadingZero(_profile) +
						'.' +
						addLeadingZero(_level) +
						'.' +
				...
				case 'av01': {
					...
					var _bitDepth =
						_profile2 === 2 && highBitDepth ? (twelveBit ? 12 : 10) : highBitDepth ? 10 : 8;
					...
					codec +=
						...
						_profile2 +
						'.' +
						addLeadingZero(_level2) +
						_tierFlag +
						'.' +
						addLeadingZero(_bitDepth) +
						'.' +
						monochrome +
						'.' +
						chromaSubsamplingX +
   ...
   */
		...
		function isCodecMediaSourceSupported(codec, type, preferManagedMediaSource) {
			...
			return (_MediaSource$isTypeSu =
				MediaSource == null
					? void 0
					: MediaSource.isTypeSupported(mimeTypeForCodec(codec, type))) != null
				? _MediaSource$isTypeSu
				: false;
		}
		...
		function getCodecCompatibleNameLower(lowerCaseCodec, preferManagedMediaSource) {
			...
			for (var i = 0; i < codecsToCheck.length; i++) {
				...
				if (isCodecMediaSourceSupported(codecsToCheck[i], 'audio', preferManagedMediaSource)) {
					...
					codecsToCheck[i] === 'mp3' &&
					(_getMediaSource = getMediaSource(preferManagedMediaSource)) != null &&
					_getMediaSource.isTypeSupported('audio/mpeg')
				) {
					return '';
				}
			}
		...
		var M3U8Parser = /*#__PURE__*/ (function () {
			...
			M3U8Parser.parseMasterPlaylist = function parseMasterPlaylist(string, baseurl) {
				...
				while ((result = MASTER_PLAYLIST_REGEX.exec(string)) != null) {
					if (result[1]) {
						...
						var level = {
							attrs: attrs,
							bitrate:
								attrs.decimalInteger('BANDWIDTH') || attrs.decimalInteger('AVERAGE-BANDWIDTH'),
							name: attrs.NAME,
						...
						if (
							!((_level$unknownCodecs = level.unknownCodecs) != null && _level$unknownCodecs.length)
						) {
				...
				var stripUnknownCodecLevels =
					levelsWithKnownCodecs.length > 0 && levelsWithKnownCodecs.length < parsed.levels.length;
					...
					if (duration) {
						...
						switch (tag) {
							...
							case 'SKIP': {
								...
								var recentlyRemovedDateranges = skipAttrs.enumeratedString(
							...
							case 'SERVER-CONTROL': {
								...
								level.canSkipDateRanges =
									level.canSkipUntil > 0 && serverControlAttrs.bool('CAN-SKIP-DATERANGES');
		...
		function parseKey(keyTagAttributes, baseurl, parsed) {
			...
			var decryptkeyformat =
				(_keyAttrs$KEYFORMAT = keyAttrs.KEYFORMAT) != null ? _keyAttrs$KEYFORMAT : 'identity';
		...
		function setFragLevelKeys(frag, levelkeys, level) {
			...
			if (
				(!encryptedFragments.length ||
					encryptedFragments[encryptedFragments.length - 1].levelkeys !== levelkeys) &&
				Object.keys(levelkeys).some(function (format) {
		...
		var PlaylistLoader = /*#__PURE__*/ (function () {
			...
			_proto.load = function load(context) {
				...
				if (
					isFiniteNumber(
						(_context$deliveryDire = context.deliveryDirectives) == null
							? void 0
							: _context$deliveryDire.part
					)
				) {
					var levelDetails;
					...
					if (levelDetails) {
						...
						if (partTarget && targetDuration) {
							var maxLowLatencyPlaylistRefresh =
				...
				var loaderCallbacks = {
					onSuccess: function onSuccess(response, stats, context, networkDetails) {
						...
						if (string.indexOf('#EXTM3U') !== 0) {
							_this.handleManifestParsingError(
								response,
								context,
						...
						if (M3U8Parser.isMediaPlaylist(string)) {
							_this.handleTrackOrLevelPlaylist(
								response,
								stats,
								context,
								networkDetails || null,
			...
			_proto.handleTrackOrLevelPlaylist = function handleTrackOrLevelPlaylist(
				response,
				stats,
				context,
				networkDetails,
				...
				var levelDetails = M3U8Parser.parseLevelPlaylist(
					response.data,
					url,
					levelId,
					levelType,
			...
			_proto.handleManifestParsingError = function handleManifestParsingError(
				response,
				context,
				error,
				networkDetails,
			...
			_proto.handleNetworkError = function handleNetworkError(
				context,
				networkDetails,
				timeout,
				response,
				...
				var message =
					'A network ' +
					(timeout ? 'timeout' : 'error' + (response ? ' (status ' + response.code + ')' : '')) +
			...
			_proto.handlePlaylistLoaded = function handlePlaylistLoaded(
				levelDetails,
				response,
				stats,
				context,
		...
		var ID3TrackController = /*#__PURE__*/ (function () {
			...
			_proto.onBufferFlushing = function onBufferFlushing(event, _ref) {
				...
				if (id3Track && (enableEmsgMetadataCues || enableID3MetadataCues)) {
					...
					if (type === 'audio') {
						...
						predicate = function predicate(cue) {
							return (
								(cue.type === MetadataSchema.audioId3 && enableID3MetadataCues) ||
								(cue.type === MetadataSchema.emsg && enableEmsgMetadataCues)
							);
						};
					}
			...
			_proto.onLevelUpdated = function onLevelUpdated(event, _ref2) {
				...
				if (
					ids.length === 0 ||
					!isFiniteNumber(lastFragment == null ? void 0 : lastFragment.programDateTime)
				) {
					return;
				}
				if (!this.id3Track) {
				...
				var _loop2 = function _loop2() {
					...
					var durationKnown =
						(appendedDateRangeCues == null ? void 0 : appendedDateRangeCues.durationKnown) || false;
						...
						var nextDateRangeWithSameClass = ids.reduce(function (candidateDateRange, id) {
							if (id !== dateRange.id) {
								...
								if (
									...
									(!candidateDateRange || dateRange.startDate < candidateDateRange.startDate)
								) {
					...
					for (var j = 0; j < attributes.length; j++) {
						...
						if (cue) {
							...
							var _cue = createCueWithDataFields(
								Cue,
								startTime,
								endTime,
								{
		...
		var LatencyController = /*#__PURE__*/ (function () {
			function LatencyController(hls) {
				...
				this.onTimeupdate = function () {
					...
					var liveMinLatencyDuration = Math.min(
						_this.maxLatency,
					...
					if (inLiveRange && distanceFromTarget > 0.05 && _this.forwardBufferLength > 1) {
						...
						var rate =
							Math.round(
								(2 / (1 + Math.exp(-0.75 * distanceFromTarget - _this.edgeStalled))) * 20
							) / 20;
			...
			_createClass(LatencyController, [
				...
				},
				{
					...
					get: function get() {
						...
						return (
							(bufferedRanges ? media.buffered.end(bufferedRanges - 1) : levelDetails.edge) -
							this.currentTime
						);
					}
				}
			...
			if (currentInitSegment) {
				...
				fragmentsToCheck.forEach(function (frag) {
					...
					if (
						frag &&
						(!frag.initSegment ||
							frag.initSegment.relurl ===
								((_currentInitSegment = currentInitSegment) == null
									? void 0
									: _currentInitSegment.relurl))
					) {
						frag.initSegment = currentInitSegment;
					}
				});
				...
				if (dateRange.isValid) {
					...
					logger.warn(
						'Ignoring invalid Playlist Delta Update DATERANGE tag: "' +
							JSON.stringify(deltaDateRanges[id].attr) +
							'"'
					);
				}
			});
			...
			if (oldParts && newParts) {
				...
				for (var i = 0, len = oldParts.length; i <= len; i++) {
					...
					if (
						_oldPart &&
						_newPart &&
						_oldPart.index === _newPart.index &&
						_oldPart.fragment.sn === _newPart.fragment.sn
					) {
		...
		function mapFragmentIntersection(oldDetails, newDetails, intersectionFn) {
			...
			var end =
				...
				(skippedSegments ? newDetails.endSN : Math.min(oldDetails.endSN, newDetails.endSN)) -
				newDetails.startSN;
		...
		function getPartWith(level, sn, partIndex) {
			...
			return findPart(
				(_level$details = level.details) == null ? void 0 : _level$details.partList,
				sn,
				partIndex
			);
		}
		...
		function shouldRetry(retryConfig, retryCount, isTimeout, loaderResponse) {
			...
			var retry =
				retryCount < retryConfig.maxNumRetry && (retryForHttpStatus(httpStatus) || !!isTimeout);
		...
		function findFragmentByPTS(fragPrevious, fragments, bufferEnd, maxFragLookUpTolerance) {
			...
			var foundFragment = BinarySearch.search(
				fragments,
		...
		var ErrorController = /*#__PURE__*/ (function (_Logger) {
			...
			_proto.getVariantLevelIndex = function getVariantLevelIndex(frag) {
				return (frag == null ? void 0 : frag.type) === PlaylistLevelType.MAIN
					? frag.level
					: this.hls.loadLevel;
			};
			_proto.onManifestLoading = function onManifestLoading() {
				...
				switch (data.details) {
					...
					case ErrorDetails.LEVEL_PARSING_ERROR:
						{
							...
							if (
								...
								!!(
									(_data$context = data.context) != null &&
									(_data$context$levelDe = _data$context.levelDetails) != null &&
									_data$context$levelDe.live
								)
							) {
					...
					case ErrorDetails.BUFFER_APPEND_ERROR:
						data.errorAction = this.getLevelSwitchAction(
							data,
							(_data$level = data.level) != null ? _data$level : hls.loadLevel
						);
						return;
			...
			_proto.getPlaylistRetryOrSwitchAction = function getPlaylistRetryOrSwitchAction(
				data,
				levelIndex
			) {
				var hls = this.hls;
			...
			_proto.getLevelSwitchAction = function getLevelSwitchAction(data, levelIndex) {
				...
				if (level) {
					...
					var findAudioCodecAlternate =
						isAudioCodecError &&
						levels.some(function (_ref) {
							var audioCodec = _ref.audioCodec;
							return level.audioCodec !== audioCodec;
						});
					// Find alternate video codec if available on video codec error
					var isVideoCodecError =
						data.sourceBufferName === 'video' &&
						(errorDetails === ErrorDetails.BUFFER_ADD_CODEC_ERROR ||
							errorDetails === ErrorDetails.BUFFER_APPEND_ERROR);
					var findVideoCodecAlternate =
						isVideoCodecError &&
						levels.some(function (_ref2) {
					...
					var _loop = function _loop() {
							...
							if (
								...
								if (errorDetails === ErrorDetails.FRAG_GAP && data.frag) {
									...
									if (levelDetails) {
										var fragCandidate = findFragmentByPTS(
											data.frag,
									...
									(fragErrorType === PlaylistLevelType.AUDIO &&
										(_level$audioGroups = level.audioGroups) != null &&
										_level$audioGroups.some(function (groupId) {
											return levelCandidate.hasAudioGroup(groupId);
										})) ||
									(fragErrorType === PlaylistLevelType.SUBTITLE &&
										(_level$subtitleGroups = level.subtitleGroups) != null &&
										_level$subtitleGroups.some(function (groupId) {
											return levelCandidate.hasSubtitleGroup(groupId);
										})) ||
									(findAudioCodecAlternate && level.audioCodec === levelCandidate.audioCodec) ||
									(!findAudioCodecAlternate && level.audioCodec !== levelCandidate.audioCodec) ||
									(findVideoCodecAlternate && level.codecSet === levelCandidate.codecSet)
								) {
			...
			_proto.onErrorOut = function onErrorOut(event, data) {
				...
				switch (
					(_data$errorAction = data.errorAction) == null ? void 0 : _data$errorAction.action
				) {
		...
		var BasePlaylistController = /*#__PURE__*/ (function (_Logger) {
			...
			_proto.switchParams = function switchParams(playlistUri, previous) {
				...
				if (renditionReports) {
					...
					if (foundIndex !== -1) {
						...
						var msn =
							parseInt(_attr['LAST-MSN']) || (previous == null ? void 0 : previous.lastPartSn);
						var part =
							parseInt(_attr['LAST-PART']) || (previous == null ? void 0 : previous.lastPartIndex);
						if (this.hls.config.lowLatencyMode) {
							var currentGoal = Math.min(
			...
			_proto.shouldReloadPlaylist = function shouldReloadPlaylist(playlist) {
				return (
					this.timer === -1 && this.requestScheduled === -1 && this.shouldLoadPlaylist(playlist)
				);
			};
				...
				if (details.live || (previousDetails != null && previousDetails.live)) {
					...
					if (details.canBlockReload && details.endSN && details.advanced) {
						...
						if (currentGoal > 0) {
							if (cdnAge > details.targetduration * 3) {
								...
								this.log(
									'Playlist last advanced ' +
										lastAdvanced.toFixed(2) +
					...
					if (msn !== undefined && details.canBlockReload) {
						this.requestScheduled =
							stats.loading.first + reloadInterval - (details.partTarget * 1000 || 1000);
					...
					this.log(
						'reload live playlist ' + index + ' in ' + Math.round(estimatedTimeUntilUpdate) + ' ms'
					);
					// this.log(
			...
			_proto.getDeliveryDirectives = function getDeliveryDirectives(
				details,
				previousDeliveryDirectives,
				msn,
				part
			) {
				var skip = getSkipValue(details, msn);
				if (
					previousDeliveryDirectives != null &&
					previousDeliveryDirectives.skip &&
			...
			_proto.checkRetry = function checkRetry(errorEvent) {
				...
				var retry =
					...
					!!retryConfig &&
					(action === NetworkErrorAction.RetryRequest ||
						(!errorAction.resolved && action === NetworkErrorAction.SendAlternateToPenaltyBox));
				if (retry) {
					...
					if (
						isTimeout &&
						(_errorEvent$context = errorEvent.context) != null &&
						_errorEvent$context.deliveryDirectives
					) {
		...
		function getVideoSelectionOptions(currentVideoRange, videoPreference) {
			...
			if (videoPreference) {
				...
				preferHDR =
					videoPreference.preferHDR !== undefined ? videoPreference.preferHDR : isHdrSupported();
				if (preferHDR) {
		...
		function getStartCodecTier(
			...
			var codecSet = codecSets.reduce(function (selected, candidate) {
					...
					if (!candidateTier.channels[channelsPreference]) {
						logStartCodecCandidateIgnored(
							...
							'no renditions with ' +
								...
								Object.keys(candidateTier.channels) +
								')'
						);
						return selected;
					}
				...
				if (
					!videoRanges.some(function (range) {
						return candidateTier.videoRanges[range] > 0;
					})
				) {
					logStartCodecCandidateIgnored(
						candidate,
						'no variants with VIDEO-RANGE of ' + JSON.stringify(videoRanges) + ' found'
					);
					return selected;
				}
		...
		function getAudioTracksByGroup(allAudioTracks) {
			return allAudioTracks.reduce(
				function (audioTracksByGroup, track) {
						...
						tier.hasDefaultAudio =
							tier.hasDefaultAudio || audioTracksByGroup.hasDefaultAudio
								...
								: audioGroup.hasAutoSelect ||
									(!audioTracksByGroup.hasDefaultAudio && !audioTracksByGroup.hasAutoSelectAudio);
        ...
        */
				_this._abandonRulesCheck = function () {
					...
					if (
						_this.getBwEstimate() * _this.hls.config.abrBandWidthUpFactor >
					...
					_this.warn(
						'Fragment ' +
							...
							(ttfb | 0) +
							' ms\n      Current BW estimate: ' +
							(isFiniteNumber(bwEstimate) ? bwEstimate | 0 : 'Unknown') +
							' bps\n      New BW estimate: ' +
							(_this.getBwEstimate() | 0) +
							' bps\n      Switching to level ' +
							nextLoadLevel +
							' @ ' +
							(nextLoadLevelBitrate | 0) +
							' bps'
					);
			...
			_proto.getTimeToLoadFrag = function getTimeToLoadFrag(
				timeToFirstByteSec,
				bandwidth,
				fragSizeBits,
				isSwitch
			...
			_proto.getNextABRAutoLevel = function getNextABRAutoLevel() {
				...
				if (bufferStarvationDelay) {
					var _bestLevel = this.findBestLevel(
						avgbw,
						minAutoLevel,
						maxAutoLevel,
				...
				if (
					(minLevel == null ? void 0 : minLevel.bitrate) <
					(autoLevel == null ? void 0 : autoLevel.bitrate)
				) {
					return minAutoLevel;
				}
			...
			_proto.findBestLevel = function findBestLevel(
				currentBw,
				minAutoLevel,
				maxAutoLevel,
				bufferStarvationDelay,
				...
				if (firstSelection) {
					...
					var startTier = getStartCodecTier(
						codecTiers,
						currentVideoRange,
						currentBw,
						audioPreference,
				...
				var _loop = function _loop() {
						...
						if (
							...
							(!upSwitch && currentFrameRate > 0 && currentFrameRate < levelInfo.frameRate) ||
							(levelInfo.supportedResult &&
								!(
									(_levelInfo$supportedR = levelInfo.supportedResult.decodingInfoResults) != null &&
									_levelInfo$supportedR[0].smooth
								))
						) {
						...
						var bitrate =
							...
							maxStarvationDelay === 0
								...
								: levels[i].maxBitrate;
						var fetchDuration = _this2.getTimeToLoadFrag(
							ttfbEstimateSec,
							adjustedbw,
							bitrate * avgDuration,
						...
						var canSwitchWithinTolerance =
							...
							(fetchDuration <= ttfbEstimateSec ||
								!isFiniteNumber(fetchDuration) ||
								(live && !_this2.bitrateTestDelay) ||
							...
							if (i !== loadLevel && (forcedAutoLevel === -1 || forcedAutoLevel !== loadLevel)) {
								if (levelsSkipped.length) {
									_this2.trace(
										'Skipped level(s) ' +
											...
											'" ' +
											levels[levelsSkipped[0]].videoRange +
											'; not compatible with "' +
											level.codecs +
											'" ' +
											currentVideoRange
								...
								_this2.info(
									'switch candidate:' +
										...
										')-bitrate=' +
										Math.round(adjustedbw - bitrate) +
										' ttfb:' +
										ttfbEstimateSec.toFixed(1) +
										' avgDuration:' +
										avgDuration.toFixed(1) +
										' maxFetchDuration:' +
										maxFetchDuration.toFixed(1) +
										' fetchDuration:' +
										fetchDuration.toFixed(1) +
										' firstSelection:' +
										firstSelection +
										' codecSet:' +
										currentCodecSet +
			...
			_createClass(AbrController, [
				{
					...
					get: function get() {
						...
						var abrAutoLevel = this.findBestLevel(
							bwEstimate,
							minAutoLevel,
							maxAutoLevel,
							0,
				...
				},
				{
					...
					get: function get() {
						...
						if (
							forcedAutoLevel !== -1 &&
							(!useEstimate || !loadedFirstFrag || this.nextAutoLevelKey === this.getAutoLevelKey())
						) {
							return forcedAutoLevel;
						}

						// compute next level using ABR logic
						var nextABRAutoLevel =
							useEstimate && loadedFirstFrag ? this.getNextABRAutoLevel() : this.firstAutoLevel;
		...
		var BufferController = /*#__PURE__*/ (function (_Logger) {
				...
				if (media && MediaSource) {
					...
					this.log(
						'created media source: ' +
							((_ms$constructor = ms.constructor) == null ? void 0 : _ms$constructor.name)
					);
			...
			_proto.resetBuffer = function resetBuffer(type) {
				...
				try {
					if (sb) {
						...
						if (
							(_this$mediaSource = this.mediaSource) != null &&
							_this$mediaSource.sourceBuffers.length
						) {
			...
			_proto.onBufferCodecs = function onBufferCodecs(event, data) {
				...
				trackNames.forEach(function (trackName) {
					if (sourceBufferCount) {
						...
						if (
							track &&
							typeof ((_track$buffer = track.buffer) == null
								? void 0
								: _track$buffer.changeType) === 'function'
						) {
							var _trackCodec;
							...
							var currentCodec =
								currentCodecFull == null
									? void 0
									: currentCodecFull.replace(VIDEO_CODEC_PROFILE_REPLACE, '$1');
							var trackCodec = pickMostCompleteCodecName(codec, levelCodec);
							var nextCodec =
								(_trackCodec = trackCodec) == null
									? void 0
									: _trackCodec.replace(VIDEO_CODEC_PROFILE_REPLACE, '$1');
			...
			_proto.blockAudio = function blockAudio(partOrFrag) {
				...
				var atGap =
					((_this$fragmentTracker = this.fragmentTracker.getAppendedFrag(
						pStart,
						PlaylistLevelType.MAIN
					)) == null
						? void 0
						: _this$fragmentTracker.gap) === true;
				if (atGap) {
					return;
				}
				var op = {
					execute: function execute() {
						...
						if (
							_this5.lastVideoAppendEnd > pTime ||
							(_this5.sourceBuffer.video &&
								BufferHelper.isBuffered(_this5.sourceBuffer.video, pTime)) ||
							((_this5$fragmentTracke = _this5.fragmentTracker.getAppendedFrag(
								pTime,
								PlaylistLevelType.MAIN
							)) == null
								? void 0
								: _this5$fragmentTracke.gap) === true
						) {
			...
			_proto.onBufferAppending = function onBufferAppending(event, eventData) {
				...
				if (
					type === 'audio' &&
					(audioTrack == null ? void 0 : audioTrack.container) === 'audio/mpeg'
				) {
            ...
            */
							_this6.warn(
								'Failed ' +
									appendErrorCount +
									'/' +
									_this6.hls.config.appendErrorMaxRetry +
			...
			_proto.trimBuffers = function trimBuffers() {
				...
				if (isFiniteNumber(backBufferLength) && backBufferLength > 0) {
					...
					var targetBackBufferPosition =
						Math.floor(currentTime / targetDuration) * targetDuration - maxBackBufferLength;
					this.flushBackBuffer(currentTime, targetDuration, targetBackBufferPosition);
				}
				if (
					isFiniteNumber(config.frontBufferFlushThreshold) &&
					config.frontBufferFlushThreshold > 0
				) {
					var frontBufferLength = Math.max(
						config.maxBufferLength,
						config.frontBufferFlushThreshold
					);
					var maxFrontBufferLength = Math.max(frontBufferLength, targetDuration);
					var targetFrontBufferPosition =
						Math.floor(currentTime / targetDuration) * targetDuration + maxFrontBufferLength;
			...
			_proto.checkPendingTracks = function checkPendingTracks() {
				...
				if (
					pendingTracksCount &&
					(!bufferCodecEventsExpected || pendingTracksCount === 2 || 'audiovideo' in pendingTracks)
				) {
			...
			_proto._onSBUpdateEnd = function _onSBUpdateEnd(type) {
				...
				if (
					((_this$mediaSource2 = this.mediaSource) == null
						? void 0
						: _this$mediaSource2.readyState) === 'closed'
				) {
			...
			_proto._onSBUpdateError = function _onSBUpdateError(type, event) {
				...
				var error = new Error(
					type +
						...
						((_this$mediaSource3 = this.mediaSource) == null
							? void 0
							: _this$mediaSource3.readyState)
				);
			...
			_createClass(BufferController, [
				{
					...
					get: function get() {
						var _this$media;
						var media =
							((_this$media = this.media) == null ? void 0 : _this$media.firstChild) || this.media;
						...
						if (maxLevel !== this.autoLevelCapping) {
							hls.logger.log(
								'Setting autoLevelCapping to ' +
									maxLevel +
									': ' +
									levels[maxLevel].height +
									'p@' +
									levels[maxLevel].bitrate +
									' for media ' +
									this.mediaWidth +
									'x' +
									this.mediaHeight
					...
					if (this.lastTime) {
						...
						if (droppedFPS > 0) {
							...
							if (currentDropped > hls.config.fpsDroppedMonitoringThreshold * currentDecoded) {
								...
								hls.logger.warn(
									'drop FPS ratio greater than max allowed value for currentLevel: ' + currentLevel
								);
								if (
									currentLevel > 0 &&
									(hls.autoLevelCapping === -1 || hls.autoLevelCapping >= currentLevel)
								) {
		...
		var ContentSteeringController = /*#__PURE__*/ (function (_Logger) {
			...
			_proto.onError = function onError(event, data) {
				var errorAction = data.errorAction;
				if (
					(errorAction == null ? void 0 : errorAction.action) ===
					...
					if (!errorAction.resolved) {
						this.warn(
							'Could not resolve ' +
								...
								' priorities: ' +
								JSON.stringify(pathwayPriority) +
								' penalized: ' +
			...
			_proto.getPathwayForGroupId = function getPathwayForGroupId(groupId, type, defaultPathway) {
				...
				for (var i = 0; i < levels.length; i++) {
					if (
						(type === PlaylistContextType.AUDIO_TRACK && levels[i].hasAudioGroup(groupId)) ||
						(type === PlaylistContextType.SUBTITLE_TRACK && levels[i].hasSubtitleGroup(groupId))
					) {
		...
		var XhrLoader = /*#__PURE__*/ (function () {
			...
			_proto.openAndSendXhr = function openAndSendXhr(xhr, context, config) {
				...
				self.clearTimeout(this.requestTimeout);
				config.timeout =
					maxTimeToFirstByteMs && isFiniteNumber(maxTimeToFirstByteMs)
			...
			_proto.loadtimeout = function loadtimeout() {
				var _this$config;
				var retryConfig =
					(_this$config = this.config) == null ? void 0 : _this$config.loadPolicy.timeoutRetry;
				...
				if (shouldRetry(retryConfig, retryCount, true)) {
					...
					logger.warn(
						'timeout while loading ' +
							((_this$context = this.context) == null ? void 0 : _this$context.url)
					);
			...
			_proto.retry = function retry(retryConfig) {
				...
				logger.warn(
					(status ? 'HTTP Status ' + status : 'Timeout') +
						' while loading ' +
						(context == null ? void 0 : context.url) +
						', retrying ' +
						stats.retry +
						'/' +
						retryConfig.maxNumRetry +
		...
		var FetchLoader = /*#__PURE__*/ (function () {
			...
			_proto.load = function load(context, config, callbacks) {
				...
				self
					...
					.then(function (response) {
						...
						_this.requestTimeout = self.setTimeout(
							function () {
								_this.abortInternal();
								callbacks.onTimeout(stats, context, _this.response);
							},
						...
						if (onProgress && isFiniteNumber(config.highWaterMark)) {
							return _this.loadProgressively(
								response,
								stats,
								context,
			...
			_proto.loadProgressively = function loadProgressively(
				response,
				stats,
				context,
				highWaterMark,
		 ...
		 */
		var hlsDefaultConfig = _objectSpread2(
			_objectSpread2(
				{
					transcoderPluginOptions: {
						...
						multicore: false,
						loadFfmpeg: (function () {
							var _loadFfmpeg = _asyncToGenerator(
								/*#__PURE__*/ _regeneratorRuntime().mark(function _callee(_m) {
		...
		function mergeConfig(defaultConfig, userConfig, logger) {
			if (
				(userConfig.liveSyncDurationCount || userConfig.liveMaxLatencyDurationCount) &&
			...
			if (
				...
				(userConfig.liveSyncDurationCount === undefined ||
					userConfig.liveMaxLatencyDurationCount <= userConfig.liveSyncDurationCount)
			) {
				throw new Error(
				...
				if (report.length) {
					logger.warn(
						'hls.js config: "' +
							...
							policyName +
							'": ' +
							JSON.stringify(userConfig[policyName])
					);
				}
			});
		...
		var LevelController = /*#__PURE__*/ (function (_BasePlaylistControll) {
			...
			_proto.onManifestLoaded = function onManifestLoaded(event, data) {
				...
				data.levels.forEach(function (levelParsed) {
					...
					if (
						(unknownCodecs != null && unknownCodecs.length) ||
						(audioCodec &&
							!areCodecsMediaSourceSupported(audioCodec, 'audio', preferManagedMediaSource)) ||
						(videoCodec &&
							!areCodecsMediaSourceSupported(videoCodec, 'video', preferManagedMediaSource))
					) {
						return;
					}
			...
			_proto.filterAndSortMediaOptions = function filterAndSortMediaOptions(
				filteredLevels,
				data,
				resolutionFound,
				videoCodecFound,
							...
							if (data.levels.length) {
								_this2.warn(
									'One or more CODECS in variant not supported: ' +
										JSON.stringify(data.levels[0].attrs)
								);
							}
				...
				if (data.audioTracks) {
					...
					audioTracks = data.audioTracks.filter(function (track) {
						return (
							!track.audioCodec ||
							areCodecsMediaSourceSupported(track.audioCodec, 'audio', preferManagedMediaSource)
						);
					});
				...
				for (var _i = 0; _i < levels.length; _i++) {
					if (levels[_i] === firstLevelInPlaylist) {
						...
						if (
							((_this$hls$userConfig = this.hls.userConfig) == null
								? void 0
								: _this$hls$userConfig.abrEwmaDefaultEstimate) === undefined
						) {
							var startingBwEstimate = Math.min(
								firstLevelBitrate,
			...
			_proto.onFragBuffered = function onFragBuffered(event, _ref2) {
				...
				if (frag !== undefined && frag.type === PlaylistLevelType.MAIN) {
					...
					if (
						!Object.keys(el).some(function (type) {
							return !!el[type];
						})
					) {
						return;
			...
			_proto.onLevelLoaded = function onLevelLoaded(event, data) {
				...
				if (!curLevel) {
					...
					if (
						(_data$deliveryDirecti = data.deliveryDirectives) != null &&
						_data$deliveryDirecti.skip
					) {
				...
				if (level === this.currentLevelIndex) {
					...
					(_data$deliveryDirecti2 = data.deliveryDirectives) != null &&
					_data$deliveryDirecti2.skip
				) {
			...
			_proto.loadPlaylist = function loadPlaylist(hlsUrlParameters) {
				...
				if (currentLevel && this.shouldLoadPlaylist(currentLevel)) {
					...
					this.log(
						'Loading level index ' +
							currentLevelIndex +
							((hlsUrlParameters == null ? void 0 : hlsUrlParameters.msn) !== undefined
			...
			_proto.removeLevel = function removeLevel(levelIndex) {
				...
				if (
					this.currentLevelIndex > -1 &&
					(_this$currentLevel = this.currentLevel) != null &&
					_this$currentLevel.details
				) {
				...
				},
				{
					...
					set: function set(newLevel) {
						...
						if (!levelDetails || levelDetails.live) {
							...
							var hlsUrlParameters = this.switchParams(
								level.uri,
								lastLevel == null ? void 0 : lastLevel.details
							);
		...
		var FragmentTracker = /*#__PURE__*/ (function () {
			...
			_proto.getFragAtPos = function getFragAtPos(position, levelType, buffered) {
				...
				for (var i = keys.length; i--; ) {
					...
					if (
						(fragmentEntity == null ? void 0 : fragmentEntity.body.type) === levelType &&
			...
			_proto.detectPartialFragments = function detectPartialFragments(data) {
				...
				Object.keys(timeRanges).forEach(function (elementaryStream) {
					...
					fragmentEntity.range[elementaryStream] = _this2.getBufferedTimes(
						frag,
						part,
						partial,
						timeRange
			...
			_proto.hasParts = function hasParts(type) {
				...
				return !!(
					(_this$activePartLists = this.activePartLists[type]) != null &&
					_this$activePartLists.length
				);
			};
			_proto.removeFragmentsInRange = function removeFragmentsInRange(
				start,
				end,
				playlistType,
				withGapOnly,
		...
		function isPartial(fragmentEntity) {
			...
			return (
				...
				(fragmentEntity.body.gap ||
					((_fragmentEntity$range = fragmentEntity.range.video) == null
						? void 0
						: _fragmentEntity$range.partial) ||
					((_fragmentEntity$range2 = fragmentEntity.range.audio) == null
						? void 0
						: _fragmentEntity$range2.partial) ||
					((_fragmentEntity$range3 = fragmentEntity.range.audiovideo) == null
						? void 0
						: _fragmentEntity$range3.partial))
			);
		}
		...
		var FragmentLoader = /*#__PURE__*/ (function () {
			...
			_proto.load = function load(frag, _onProgress) {
				...
				return new Promise(function (resolve, reject) {
					...
					if (frag.gap) {
						if (
							frag.tagList.some(function (tags) {
								return tags[0] === 'GAP';
							})
						) {
					...
					var loader =
						(_this.loader =
						frag.loader =
							FragmentILoader ? new FragmentILoader(config) : new DefaultILoader(config));
		...
		function createLoaderContext(frag, part) {
			...
			if (isFiniteNumber(start) && isFiniteNumber(end)) {
				...
				if (
					frag.sn === 'initSegment' &&
					isMethodFullSegmentAesCbc(
						(_frag$decryptdata = frag.decryptdata) == null ? void 0 : _frag$decryptdata.method
					)
				) {
		...
		var KeyLoader = /*#__PURE__*/ (function () {
				...
				for (var uri in this.keyUriToKeyInfo) {
					...
					if (loader) {
						...
						if (
							type &&
							type !==
								((_loader$context = loader.context) == null ? void 0 : _loader$context.frag.type)
						) {
							return;
						}
						loader.abort();
			...
			_proto.createKeyLoadError = function createKeyLoadError(
				frag,
				details,
				error,
				networkDetails,
			...
			_proto.loadInternal = function loadInternal(frag, keySystemFormat) {
				...
				if ((_keyInfo2 = keyInfo) != null && _keyInfo2.keyLoadPromise) {
					...
					switch (
						(_keyInfo$mediaKeySess = keyInfo.mediaKeySessionContext) == null
							? void 0
							: _keyInfo$mediaKeySess.keyStatus
					) {
						case undefined:
						case 'status-pending':
						case 'usable':
					...
					default:
						return Promise.reject(
							this.createKeyLoadError(
								frag,
								ErrorDetails.KEY_LOAD_ERROR,
								new Error('Key supplied with unsupported METHOD: "' + decryptdata.method + '"')
							)
						);
				}
			};
		...
		function alignMediaPlaylistByPDT(details, refDetails) {
			...
			if (!refFrag || !frag) {
				...
				frag =
					findFirstFragWithCC(fragments, refFrag.cc) || fragments[Math.floor(fragments.length / 2)];
			}
		...
		var BaseStreamController = /*#__PURE__*/ (function (_TaskLoop) {
			...
			function BaseStreamController(hls, fragmentTracker, keyLoader, logPrefix, playlistType) {
				...
				_this.onMediaSeeking = function () {
					...
					_this.log(
						'media seeking to ' +
							(isFiniteNumber(currentTime) ? currentTime.toFixed(3) : currentTime) +
							', state: ' +
							state
					);
					...
					if (media) {
						...
						_this.fragmentTracker.removeFragmentsInRange(
							currentTime,
							Infinity,
							_this.playlistType,
							true
			...
			_proto._streamEnded = function _streamEnded(bufferInfo, levelDetails) {
				...
				if (partList != null && partList.length) {
					...
					var lastPartBuffered = BufferHelper.isBuffered(
						this.media,
					...
					.then(function (data) {
						...
						if (_this2.fragContextChanged(frag)) {
							if (
								...
								(!_this2.fragCurrent && state === State.PARSING)
							) {
								_this2.fragmentTracker.removeFragment(frag);
			...
			_proto.clearTrackerIfNeeded = function clearTrackerIfNeeded(frag) {
				...
				if (fragState === FragmentState.APPENDING) {
					...
					var minForwardBufferLength = Math.max(
						frag.duration,
					...
					((_this$mediaBuffer = this.mediaBuffer) == null
						? void 0
						: _this$mediaBuffer.buffered.length) === 0
				) {
			...
			_proto.fragContextChanged = function fragContextChanged(frag) {
				var fragCurrent = this.fragCurrent;
				return (
					!frag || !fragCurrent || frag.sn !== fragCurrent.sn || frag.level !== fragCurrent.level
				);
			};
			_proto.fragBufferedComplete = function fragBufferedComplete(frag, part) {
				...
				this.log(
					'Buffered ' +
						...
						frag.level +
						' (frag:[' +
						((_frag$startPTS = frag.startPTS) != null ? _frag$startPTS : NaN).toFixed(3) +
						'-' +
						((_frag$endPTS = frag.endPTS) != null ? _frag$endPTS : NaN).toFixed(3) +
						'] > buffer:' +
						(media ? TimeRanges.toString(BufferHelper.getBuffered(media)) : '(detached)') +
						')'
				);
				if (frag.sn !== 'initSegment') {
					var _this$levels;
				...
				if (
					...
					((_this$fragCurrent = this.fragCurrent) == null ? void 0 : _this$fragCurrent.sn) ===
						((_this$fragPrevious = this.fragPrevious) == null ? void 0 : _this$fragPrevious.sn)
				) {
			...
			_proto._handleFragmentLoadComplete = function _handleFragmentLoadComplete(fragLoadedEndData) {
				...
				var chunkMeta = new ChunkMetadata(
					frag.level,
					frag.sn,
			...
			_proto._doFragLoad = function _doFragLoad(frag, level, targetBufferTime, progressCallback) {
				...
				if (
					frag.encrypted &&
					!((_frag$decryptdata = frag.decryptdata) != null && _frag$decryptdata.key)
				) {
					this.log(
						'Loading key for ' +
							frag.sn +
					...
					if (shouldLoadParts !== this.loadingParts) {
						this.log(
							'LL-Part loading ' +
								...
								' loading sn ' +
								(fragPrevious == null ? void 0 : fragPrevious.sn) +
								'->' +
								frag.sn
						);
					...
					if (partList && progressCallback) {
						...
						if (partIndex > -1) {
							...
							if (keyLoadingPromise) {
								...
								_result = this.doFragPartsLoad(frag, part, level, progressCallback).catch(
									function (error) {
										return _this4.handleFragLoadError(error);
									}
								);
				...
				if (dataOnProgress && keyLoadingPromise) {
					result = keyLoadingPromise
						.then(function (keyLoadedData) {
							if (
								!keyLoadedData ||
								_this4.fragContextChanged(keyLoadedData == null ? void 0 : keyLoadedData.frag)
							) {
								return null;
							}
							...
							/*#__PURE__*/ (function () {
								var _ref2 = _asyncToGenerator(
									/*#__PURE__*/ _regeneratorRuntime().mark(function _callee(_ref) {
										var fragLoadedData, transcodedData;
										return _regeneratorRuntime().wrap(function _callee$(_context) {
											while (1)
												switch ((_context.prev = _context.next)) {
													case 0:
														fragLoadedData = _ref[0];
														_context.next = 3;
														return _this4.hls.transcoder.transcode(
															new Uint8Array(fragLoadedData.payload)
														);
													case 3:
														transcodedData = _context.sent;
														fragLoadedData.payload = transcodedData;
														fragLoadedData.frag.data = transcodedData;
														if (!dataOnProgress && fragLoadedData && progressCallback) {
															progressCallback(fragLoadedData);
														}
														return _context.abrupt('return', fragLoadedData);
													case 8:
													case 'end':
														return _context.stop();
												}
					...
					var loadPart = function loadPart(part) {
						_this5.fragmentLoader
							...
							.then(function (partLoadedData) {
								...
								var nextPart =
									getPartWith(level, frag.sn, part.index + 1) ||
									findPart(initialPartList, frag.sn, part.index + 1);
								if (nextPart) {
									loadPart(nextPart);
								} else {
									return resolve({
			...
			_proto.shouldLoadParts = function shouldLoadParts(details, bufferEnd) {
				if (this.config.lowLatencyMode) {
					...
					if (details != null && details.partList) {
						...
						var safePartStart =
							firstPart.end +
							(((_details$fragmentHint = details.fragmentHint) == null
								? void 0
								: _details$fragmentHint.duration) || 0);
						if (
			...
			_proto.bufferFragmentData = function bufferFragmentData(
				data,
				frag,
				part,
				chunkMeta,
			...
			_proto.getNextFragment = function getNextFragment(pos, levelDetails) {
				...
				if (levelDetails.live) {
					...
					if (
						(!levelDetails.PTSKnown && !this.startFragRequested && this.startPosition === -1) ||
						pos < start
					) {
			...
			_proto.getNextFragmentLoopLoading = function getNextFragmentLoopLoading(
				frag,
				levelDetails,
				bufferInfo,
				playlistType,
				...
				if (gapStart && frag && !frag.gap && bufferInfo.nextStart) {
					...
					var nextbufferInfo = this.getFwdBufferInfoAtPos(
			...
			_proto.mapToInitFragWhenRequired = function mapToInitFragWhenRequired(frag) {
				...
				if (
					frag != null &&
					frag.initSegment &&
					!(frag != null && frag.initSegment.data) &&
					!this.bitrateTest
				) {
     ...
     */
			...
			_proto.waitForCdnTuneIn = function waitForCdnTuneIn(details) {
				...
				return (
					...
					details.partTarget &&
					details.tuneInGoal > Math.max(details.partHoldBack, details.partTarget * advancePartLimit)
				);
			};
			...
			_proto.resetFragmentLoading = function resetFragmentLoading(frag) {
				if (
					!this.fragCurrent ||
					(!this.fragContextChanged(frag) && this.state !== State.FRAG_LOADING_WAITING_RETRY)
				) {
					this.state = State.IDLE;
				}
			};
			_proto.onFragmentOrKeyLoadError = function onFragmentOrKeyLoadError(filterType, data) {
				...
				if (this.fragContextChanged(frag)) {
					...
					this.warn(
						'Frag load error must match current frag to retry ' +
							frag.url +
							' > ' +
							((_this$fragCurrent2 = this.fragCurrent) == null ? void 0 : _this$fragCurrent2.url)
					);
					return;
				}
				...
				var errorAction = data.errorAction;
				var _ref3 = errorAction || {},
					action = _ref3.action,
					_ref3$retryCount = _ref3.retryCount,
					retryCount = _ref3$retryCount === void 0 ? 0 : _ref3$retryCount,
			...
			_proto.updateLevelTiming = function updateLevelTiming(frag, part, level, partial) {
				...
				if (
					!parsed &&
					((_this$transmuxer = this.transmuxer) == null ? void 0 : _this$transmuxer.error) === null
				) {
					var error = new Error(
		...
		function isSupported() {
			...
			var mediaSource = getMediaSource();
			return (
				typeof (mediaSource == null ? void 0 : mediaSource.isTypeSupported) === 'function' &&
				(['avc1.42E01E,mp4a.40.2', 'av01.0.01M.08', 'vp09.00.50.08'].some(
					function (codecsForVideoContainer) {
		...
		function changeTypeSupported() {
			...
			var sourceBuffer = getSourceBuffer();
			return (
				typeof (sourceBuffer == null
					? void 0
					: (_sourceBuffer$prototy = sourceBuffer.prototype) == null
						? void 0
						: _sourceBuffer$prototy.changeType) === 'function'
			);
		}
      ...
      */
				...
				if (
					(audioCodec &&
						(audioCodec.indexOf('mp4a.40.29') !== -1 || audioCodec.indexOf('mp4a.40.5') !== -1)) ||
    ...
    */
		...
		var AACDemuxer = /*#__PURE__*/ (function (_BaseAudioDemuxer) {
			...
			_proto.resetInitSegment = function resetInitSegment(
				...
				_BaseAudioDemuxer.prototype.resetInitSegment.call(
					this,
					initSegment,
					audioCodec,
					videoCodec,
					...
					if (emsgs) {
						emsgs.forEach(function (data) {
							...
							if (emsgSchemePattern.test(emsgInfo.schemeIdUri)) {
								var pts = isFiniteNumber(emsgInfo.presentationTime)
				...
				units.forEach(function (unit) {
					...
					switch (unit.type) {
						...
						case 1: {
							...
							if (!VideoSample) {
								VideoSample = _this.VideoSample = _this.createVideoSample(
									true,
									pes.pts,
									pes.dts,
									''
						...
						case 5:
							...
							if (
								(_VideoSample2 = VideoSample) != null &&
								_VideoSample2.frame &&
								!VideoSample.key
							) {
								_this.pushAccessUnit(VideoSample, track);
						...
						case 7: {
							...
							if (
								...
								((_track$pixelRatio = track.pixelRatio) == null ? void 0 : _track$pixelRatio[0]) !==
									config.pixelRatio[0] ||
								((_track$pixelRatio2 = track.pixelRatio) == null
									? void 0
									: _track$pixelRatio2[1]) !== config.pixelRatio[1]
							) {
								track.width = config.width;
								track.height = config.height;
		...
		var TSDemuxer = /*#__PURE__*/ (function () {
			...
			_proto.demuxSampleAes = function demuxSampleAes(data, keyData, timeOffset) {
				...
				var sampleAes = (this.sampleAes = new SampleAesDecrypter(
					this.observer,
					this.config,
					keyData
				));
            ...
            as Bitwise operators treat their operands as a sequence of 32 bits */
					...
					if (pesFlags & 0x40) {
						...
						if (pesPts - pesDts > 60 * 90000) {
							logger.warn(
								Math.round((pesPts - pesDts) / 90000) + 's delta between PTS and DTS, align them'
							);
							pesPts = pesDts;
						}
					} else {
				...
				switch (codec) {
					case 'mp4a.40.2':
						if (channelCount === 1) {
							...
							return new Uint8Array([
								0x00, 0xc8, 0x00, 0x80, 0x20, 0x84, 0x01, 0x26, 0x40, 0x08, 0x64, 0x00, 0x80, 0x2c,
								0x80, 0x08, 0x02, 0x38
							]);
						} else if (channelCount === 5) {
							return new Uint8Array([
								0x00, 0xc8, 0x00, 0x80, 0x20, 0x84, 0x01, 0x26, 0x40, 0x08, 0x64, 0x00, 0x82, 0x30,
								0x04, 0x99, 0x00, 0x21, 0x90, 0x02, 0x38
							]);
						} else if (channelCount === 6) {
							return new Uint8Array([
								0x00, 0xc8, 0x00, 0x80, 0x20, 0x84, 0x01, 0x26, 0x40, 0x08, 0x64, 0x00, 0x82, 0x30,
								0x04, 0x99, 0x00, 0x21, 0x90, 0x02, 0x00, 0xb2, 0x00, 0x20, 0x08, 0xe0
							]);
						}
						break;
					...
					default:
						if (channelCount === 1) {
							...
							return new Uint8Array([
								0x1, 0x40, 0x22, 0x80, 0xa3, 0x4e, 0xe6, 0x80, 0xba, 0x8, 0x0, 0x0, 0x0, 0x1c, 0x6,
								0xf1, 0xc1, 0xa, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a,
							...
							return new Uint8Array([
								0x1, 0x40, 0x22, 0x80, 0xa3, 0x5e, 0xe6, 0x80, 0xba, 0x8, 0x0, 0x0, 0x0, 0x0, 0x95,
								0x0, 0x6, 0xf1, 0xa1, 0xa, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a,
								0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a,
								0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a, 0x5a,
								0x5a, 0x5e
							]);
		...
		var MP4 = /*#__PURE__*/ (function () {
			...
			MP4.hvc1 = function hvc1(track) {
				...
				for (var _i = 0; _i < units.length; _i += 1) {
					hvcC.set(
						new Uint8Array([(32 + _i) | (_i === iMax ? 128 : 0), 0x00, units[_i].length]),
						length
					);
					length += 3;
					for (var _j = 0; _j < units[_i].length; _j += 1) {
						hvcC.set(
							new Uint8Array([units[_i][_j].length >> 8, units[_i][_j].length & 255]),
							length
						);
						length += 2;
		...
		var MP4Remuxer = /*#__PURE__*/ (function () {
			...
			_proto.remux = function remux(
				...
				var canRemuxAvc =
					((!hasAudio || enoughAudioSamples) && (!hasVideo || enoughVideoSamples)) ||
					this.ISGenerated ||
					flush;
				if (canRemuxAvc) {
					if (this.ISGenerated) {
						var _videoTrack$pixelRati,
							_config$pixelRatio,
							_videoTrack$pixelRati2,
							_config$pixelRatio2;
						...
						if (
							...
							(videoTrack.width !== config.width ||
								...
								((_videoTrack$pixelRati = videoTrack.pixelRatio) == null
									? void 0
									: _videoTrack$pixelRati[0]) !==
									((_config$pixelRatio = config.pixelRatio) == null
										? void 0
										: _config$pixelRatio[0]) ||
								((_videoTrack$pixelRati2 = videoTrack.pixelRatio) == null
									? void 0
									: _videoTrack$pixelRati2[1]) !==
									((_config$pixelRatio2 = config.pixelRatio) == null
										? void 0
										: _config$pixelRatio2[1]))
						) {
					...
					if (enoughVideoSamples) {
						...
						if (!isVideoContiguous && this.config.forceKeyFrameOnDiscontinuity) {
							...
							if (firstKeyFrameIndex > 0) {
								...
								videoTimeOffset +=
									(videoTrack.samples[0].pts - startPTS) / videoTrack.inputTimeScale;
					...
					if (this.ISGenerated) {
						...
						if (enoughAudioSamples) {
							...
							audio = this.remuxAudio(
								...
								accurateTimeOffset,
								hasVideo || enoughVideoSamples || playlistType === PlaylistLevelType.AUDIO
									? videoTimeOffset
									: undefined
							);
							if (enoughVideoSamples) {
			...
			_proto.remuxVideo = function remuxVideo(track, timeOffset, contiguous, audioTrackLength) {
				...
				if (contiguous) {
					...
					if (foundHole || foundOverlap) {
						if (foundHole) {
							logger.warn(
								(track.segmentCodec || '').toUpperCase() +
									': ' +
        ...
        (need 8 more bytes to fill length and mpdat type) */
				...
				for (var _i3 = 0; _i3 < nbSamples; _i3++) {
					...
					if (_i3 < nbSamples - 1) {
						...
						var lastFrameDuration =
							_i3 > 0 ? _VideoSample.dts - inputSamples[_i3 - 1].dts : averageSampleDuration;
						ptsDelta =
							_i3 > 0 ? _VideoSample.pts - inputSamples[_i3 - 1].pts : averageSampleDuration;
						if (config.stretchShortVideoTrack && this.nextAudioPts !== null) {
							...
							var deltaToFrameEnd =
								(audioTrackLength ? minPTS + audioTrackLength * timeScale : this.nextAudioPts) -
								_VideoSample.pts;
						...
						if (
							...
							for (var _i4 = 0, len = outputSamples.length; _i4 < len; _i4++) {
								...
								if (_i4 < len - 1) {
									...
									outputSamples[_i4].duration = _i4
										? outputSamples[_i4 - 1].duration
				...
				mp4SampleDuration =
					stretchedLastFrame || !mp4SampleDuration ? averageSampleDuration : mp4SampleDuration;
			...
			_proto.remuxAudio = function remuxAudio(
				...
				this.isAudioContiguous = contiguous =
					...
					(inputSamples.length &&
						nextAudioPts > 0 &&
						((accurateTimeOffset && Math.abs(timeOffsetMpegTS - nextAudioPts) < 9000) ||
							Math.abs(
								normalizePts(inputSamples[0].pts - initTime, timeOffsetMpegTS) - nextAudioPts
							) <
					...
					for (var i = 0, nextPts = nextAudioPts; i < inputSamples.length; i++) {
						...
						if (delta <= -maxAudioFramesDrift * inputSampleDuration && alignedWithVideo) {
							if (i === 0) {
								logger.warn(
									'Audio frame @ ' +
										(pts / inputTimeScale).toFixed(3) +
										's overlaps nextAudioPts by ' +
										Math.round((1000 * delta) / inputTimeScale) +
										' ms.'
								);
						...
						// 4: remuxing with video (videoTimeOffset !== undefined)
							...
							logger.warn(
								'[mp4-remuxer]: Injecting ' +
									missing +
									' audio frame @ ' +
									(nextPts / inputTimeScale).toFixed(3) +
									's due to ' +
									Math.round((1000 * delta) / inputTimeScale) +
									' ms gap.'
							);
							for (var j = 0; j < missing; j++) {
								...
								var fillFrame = AAC.getSilentFrame(
			...
			_proto.remuxEmptyAudio = function remuxEmptyAudio(track, timeOffset, contiguous, videoData) {
				...
				var startDTS =
					(nextAudioPts !== null ? nextAudioPts : videoData.startDTS * inputTimeScale) + init90kHz;
				...
				// silent frame
				var silentFrame = AAC.getSilentFrame(
					track.parsedCodec || track.manifestCodec || track.codec,
					track.channelCount
				);
      ...
      PTS looping occured. fill the gap */
		...
		var Transmuxer = /*#__PURE__*/ (function () {
			...
			_proto.push = function push(data, decryptdata, chunkMeta, state) {
				...
				if (keyData && isFullSegmentEncryption(keyData.method)) {
					...
					if (decrypter.isSync()) {
						// Software decryption is progressive. Progressive decryption may not return a result on each call. Any cached
						// data is handled in the flush() call
						var decryptedData = decrypter.softwareDecrypt(
							uintData,
							keyData.key.buffer,
							keyData.iv.buffer,
							aesMode
						...
						this.decryptionPromise = decrypter
							.webCryptoDecrypt(uintData, keyData.key.buffer, keyData.iv.buffer, aesMode)
							.then(function (decryptedData) {
			...
			_proto.flushRemux = function flushRemux(transmuxResults, demuxResult, chunkMeta) {
				...
				var remuxResult = this.remuxer.remux(
					audioTrack,
					videoTrack,
					id3Track,
					textTrack,
			...
			_proto.transmux = function transmux(
				data,
				keyData,
				timeOffset,
				accurateTimeOffset,
			...
			_proto.transmuxUnencrypted = function transmuxUnencrypted(
				data,
				timeOffset,
				accurateTimeOffset,
				chunkMeta
			...
			_proto.transmuxSampleAes = function transmuxSampleAes(
				data,
				decryptData,
				timeOffset,
				accurateTimeOffset,
				...
				return this.demuxer
					...
					.then(function (demuxResult) {
						var remuxResult = _this3.remuxer.remux(
		...
		function getEncryptionType(data, decryptData) {
			...
			if (
				data.byteLength > 0 &&
				(decryptData == null ? void 0 : decryptData.key) != null &&
				decryptData.iv !== null &&
		...
		var TransmuxConfig = function TransmuxConfig(
			audioCodec,
			videoCodec,
			initSegmentData,
			duration,
		...
		var TransmuxState = function TransmuxState(
			discontinuity,
			contiguous,
			accurateTimeOffset,
			trackSwitch,
		...
		function startWorker(self) {
			...
			self.addEventListener('message', function (ev) {
				...
				switch (data.cmd) {
					...
					case 'demux': {
						var transmuxResult = self.transmuxer.push(
							data.data,
							data.decryptdata,
							data.chunkMeta,
							data.state
		...
		var TransmuxerInterface = /*#__PURE__*/ (function () {
			...
			_proto.push = function push(
				...
				var progressive =
					snDiff === 0 &&
					chunkMeta.id > 1 &&
					chunkMeta.id === (lastFrag == null ? void 0 : lastFrag.stats.chunkCount);
				var contiguous =
					!trackSwitch &&
					(snDiff === 1 || (snDiff === 0 && (partDiff === 1 || (progressive && partDiff <= 0))));
				var now = self.performance.now();
				...
				var initSegmentChange = !(
					lastFrag &&
					((_frag$initSegment = frag.initSegment) == null ? void 0 : _frag$initSegment.url) ===
						((_lastFrag$initSegment = lastFrag.initSegment) == null
							? void 0
							: _lastFrag$initSegment.url)
				);
				var state = new TransmuxState(
					discontinuity,
					contiguous,
					accurateTimeOffset,
					trackSwitch,
			...
			_proto.onWorkerMessage = function onWorkerMessage(ev) {
				...
				switch (data.event) {
					case 'init': {
						...
						var objectURL =
							(_this$workerContext2 = this.workerContext) == null
								? void 0
								: _this$workerContext2.objectURL;
						if (objectURL) {
							// revoke the Object URL that was used to create transmuxer worker, so as not to leak it
							self.URL.revokeObjectURL(objectURL);
						}
			...
			_proto.poll = function poll(lastCurrentTime, activeFrag, levelDetails, state) {
				...
				if (!seeking && stalledDuration >= STALL_MINIMUM_DURATION_MS) {
					...
					if (
						...
						Math.abs(currentTime - ((levelDetails == null ? void 0 : levelDetails.edge) || 0)) < 1
					) {
			...
			_proto._tryFixBufferStall = function _tryFixBufferStall(bufferInfo, stalledDurationMs) {
				...
				if (
					(bufferInfo.len > config.maxBufferHole ||
						(bufferInfo.nextStart && bufferInfo.nextStart - currentTime < config.maxBufferHole)) &&
				...
				if (startTime) {
					...
					if (gapLength > 0 && (bufferStarved || waiting)) {
						...
						if (gapLength > config.maxBufferHole) {
							...
							if (!startGap) {
								var startProvisioned =
									partial || fragmentTracker.getAppendedFrag(currentTime, PlaylistLevelType.MAIN);
								if (startProvisioned) {
		...
		var StreamController = /*#__PURE__*/ (function (_BaseStreamController) {
					...
					case State.FRAG_LOADING_WAITING_RETRY:
						{
							...
							if (
								!retryDate ||
								now >= retryDate ||
								((_this$media = this.media) != null && _this$media.seeking)
							) {
			...
			_proto.doTickIdle = function doTickIdle() {
				...
				if (
					...
					var backtrackSn = (
						(_this$backtrackFragme = this.backtrackFragment) != null ? _this$backtrackFragme : frag
					).sn;
				...
				if (frag && this.isLoopLoading(frag, targetBufferTime)) {
					...
					if (!gapStart) {
						...
						var mediaBuffer =
							(type === ElementaryStreamTypes.VIDEO ? this.videoBuffer : this.mediaBuffer) ||
							this.media;
						if (mediaBuffer) {
    ...
    */
				...
				if (media != null && media.readyState) {
					...
					if (!media.paused && levels) {
						...
						if (fragLastKbps && this.fragCurrent) {
							fetchdelay =
								(this.fragCurrent.duration * nextLevel.maxBitrate) / (1000 * fragLastKbps) + 1;
						} else {
							fetchdelay = 0;
						}
					} else {
			...
			_proto.flushMainBuffer = function flushMainBuffer(startOffset, endOffset) {
				_BaseStreamController.prototype.flushMainBuffer.call(
					this,
					startOffset,
					endOffset,
			...
			_proto.onLevelLoaded = function onLevelLoaded(event, data) {
				...
				if (
					fragCurrent &&
					(this.state === State.FRAG_LOADING || this.state === State.FRAG_LOADING_WAITING_RETRY)
				) {
				...
				if (
					newDetails.live ||
					((_curLevel$details = curLevel.details) != null && _curLevel$details.live)
				) {
					var _this$levelLastLoaded;
					...
					sliding = this.alignPlaylists(
						newDetails,
						curLevel.details,
						(_this$levelLastLoaded = this.levelLastLoaded) == null
							? void 0
							: _this$levelLastLoaded.details
					);
				}
			...
			_proto._handleFragmentLoadProgress = function _handleFragmentLoadProgress(data) {
				...
				var initSegmentData =
					(_frag$initSegment = frag.initSegment) == null ? void 0 : _frag$initSegment.data;
				...
				switch (data.details) {
					...
					case ErrorDetails.LEVEL_PARSING_ERROR:
						...
						if (
							!data.levelRetry &&
							this.state === State.WAITING_LEVEL &&
							((_data$context = data.context) == null ? void 0 : _data$context.type) ===
								PlaylistContextType.LEVEL
						) {
							this.state = State.IDLE;
						}
						break;
			...
			_proto.seekToStartPos = function seekToStartPos() {
				...
				if (startPosition >= 0 && currentTime < startPosition) {
					...
					if (
						delta > 0 &&
						(delta < this.config.maxBufferHole || delta < this.config.maxFragLookUpTolerance)
					) {
			...
			_proto._bufferInitSegment = function _bufferInitSegment(
				currentLevel,
				tracks,
				frag,
				chunkMeta
        ...
        */
									...
									typeof document === 'undefined' && typeof location === 'undefined'
										...
										: typeof document === 'undefined'
											...
											: (_documentCurrentScript && _documentCurrentScript.src) ||
												new URL('hls.light.js', document.baseURI).href
								),
								{
									type: 'module'
								}
		...
		var TranscoderPlugin = /*#__PURE__*/ (function () {
			function TranscoderPlugin(config) {
				this.ffmpeg = new FFmpeg();
				this.config = void 0;
				this.fallbackVideoData = void 0;
				this.config = config;
				console.log('%c(transhls) constructed', 'font-size: large');
				console.log(config);
			}
			var _proto = TranscoderPlugin.prototype;
								...
								while (1)
									switch ((_context.prev = _context.next)) {
										case 0:
											...
											_context.next = 3;
											return this.config.loadFfmpeg(this.config.multicore);
										case 3:
											this.ffmpeg = _context.sent;
											...
											_context.next = 8;
											return fetch(this.config.fallbackVideoUrl);
										case 8:
											_context.next = 10;
											return _context.sent.arrayBuffer();
										case 10:
											this.fallbackVideoData = _context.sent;
										case 11:
											console.log(
			...
			_proto.transcode = /*#__PURE__*/ (function () {
				var _transcode = _asyncToGenerator(
							...
							function _callee2$(_context2) {
								while (1)
									switch ((_context2.prev = _context2.next)) {
										case 0:
											console.log(
												'%c(transhls) processing payload with size ' + data.byteLength,
												'font-size: large;'
											);
											_context2.next = 3;
											return this.ffmpeg.writeFile('input.mp4', new Uint8Array(data));
										case 3:
											_context2.next = 5;
											return this.ffmpeg.exec(
												'-re -i input.mp4 -c:a aac -b:a 64k -c:v libx264 -crf 45 -preset ultrafast -movflags frag_keyframe+empty_moov+default_base_moof+omit_tfhd_offset+faststart out.mp4'.split(
													' '
												)
											);
										case 5:
											_context2.next = 7;
											return this.ffmpeg.readFile('out.mp4');
										case 7:
		...
		var Hls = /*#__PURE__*/ (function () {
			...
			function Hls(userConfig) {
				...
				if (userConfig.transcoderPluginOptions === undefined) {
					throw new Error('transhls cannot be initialized without transcoderPluginOptions');
				}
				...
				var bufferController = (this.bufferController = new ConfigBufferController(
					this,
					fragmentTracker
				));
				...
				var streamController = (this.streamController = new StreamController(
					this,
					fragmentTracker,
					keyLoader
				));
			...
			_proto.loadSource = function loadSource(url) {
				...
				var loadingSource = (this.url = urlToolkitExports.buildAbsoluteURL(
					self.location.href,
					url,
					{
						alwaysNormalize: true
			...
			_proto.setAudioOption = function setAudioOption(audioOption) {
				...
				return (_this$audioTrackContr = this.audioTrackController) == null
					? void 0
					: _this$audioTrackContr.setAudioOption(audioOption);
			};
			/**
			...
			_proto.setSubtitleOption = function setSubtitleOption(subtitleOption) {
				...
				(_this$subtitleTrackCo = this.subtitleTrackController) == null
					? void 0
					: _this$subtitleTrackCo.setSubtitleOption(subtitleOption);
				return null;
			};
